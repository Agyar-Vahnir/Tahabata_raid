...
def plot_dps(playerlist, logfile, show_asmos = True, show_elyos = True, display_mean = True, Data = "Damage"):
    ...
    if check_people.check_new_people(playerlist,logfile):
        logfile_filtered = logfile[:-4]+"_filtered.csv"
        manage_siege_weapons.filter_siege_weapons(logfile,logfile_filtered)
        ...
        with open(logfile_filtered) as newlog: 
            it_players = csv.reader(newlog,delimiter=" ",skipinitialspace=False)
            loadedlog = []
            for p_log in it_players:
                if p_log["Player"][:6] == "Siege_":
                    loadedlog.append({"Name":p_log[0],"Class": "Siege", "Faction":p_db["Faction"],"Damage":int(p_log[1]),"DPS":int(p_log[2])})
                else:
...
plot_dps("C:/Users/Salto/Documents/RAIDS/08-30-GI/log_filtered.csv", Data = "Damage")
plot_dps("C:/Users/Salto/Documents/RAIDS/08-30-GI/log_filtered.csv", Data = "DPS")
